...
## 2. 목적
시스템 관리자/IP 운영자가 일상 운영·장애·데이터·권한을 처리하는 절차를 정의한다.
...
## 5. 일상 운영
- 매일: 연동 실패/Queue/Alert/Critical Risk dashboard 확인
- 매주: Watch 미평가, Gate overdue, 권한변경 확인
- 월간: 사용자/프로젝트 접근 재검토, 백업 복구 샘플, AI 비용/품질
- 분기: 보존정책/connector/API key/권한 audit

### 장애
1. Incident 생성
2. 영향 범위(project/data/integration) 식별
3. 외부 Patent/AI 장애 시 기능 degrade
4. 데이터 오류 시 write 제한 검토
5. 복구 후 audit reconciliation
6. RCA/재발방지

### 특허데이터 장애
G6/G7 직전 법적상태 최신화가 실패하면 Gate에 `EXTERNAL_DATA_STALE` 조건을 표시하고 IP/Legal 수동 검증 후 예외승인 가능.
