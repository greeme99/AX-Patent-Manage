...
## 5. Component Set
Button / Input / Select / DatePicker / DataGrid / Tree / Tabs / Badge / RiskChip / GateStepper / EvidenceLink / VersionTag / DiffViewer / ApprovalPanel / CitationCard / AuditTimeline / EmptyState / ErrorState

### Semantic tokens
- `risk.critical`, `risk.high`, `risk.medium`, `risk.low`, `risk.cleared`
- `gate.passed`, `gate.conditional`, `gate.rejected`, `gate.pending`
- `source.human`, `source.ai`, `source.external`

### 컴포넌트 규칙
- `RiskChip`: 등급 + score + tooltip
- `VersionTag`: object type + revision + frozen 여부
- `CitationCard`: source title, patent no., claim no., excerpt, link, retrieved_at
- `ApprovalPanel`: decision, approver, role, timestamp, conditions
