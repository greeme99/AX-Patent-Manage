...
erDiagram
  ...
  PROJECT ||--o{ FTO_SCOPE : defines
  PRODUCT ||--o{ DESIGN_REVISION : versions
  PRODUCT ||--o{ FEATURE : decomposes
  PROJECT ||--o{ SEARCH_RUN : performs
  SEARCH_RUN ||--o{ PATENT : finds
  PATENT }o--|| PATENT_FAMILY : belongs
  PATENT ||--o{ CLAIM_VERSION : has
  CLAIM_VERSION ||--o{ CLAIM_ELEMENT : decomposes
  CLAIM_ELEMENT ||--o{ CLAIM_MAPPING : maps
  FEATURE ||--o{ CLAIM_MAPPING : compared
  DESIGN_REVISION ||--o{ CLAIM_MAPPING : basis
  CLAIM_MAPPING ||--o{ EVIDENCE_LINK : proves
  EVIDENCE ||--o{ EVIDENCE_LINK : referenced
  PROJECT ||--o{ RISK : owns
  PATENT ||--o{ RISK : causes
  RISK ||--o{ DECISION : resolved_by
  PROJECT ||--o{ GATE_REVIEW : controls
  GATE_REVIEW ||--o{ APPROVAL : approved_by
  PROJECT ||--o{ WATCH_RULE : monitors
  PROJECT ||--o{ AUDIT_EVENT : audits
```

### 핵심 불변조건
- CLAIM_MAPPING은 claim_version + feature + design_revision 조합을 고정 참조
- RISK는 최소 1개 Patent/Feature 또는 free-text rationale 필요
- Gate approval은 상태변경 이벤트로 Audit 기록
