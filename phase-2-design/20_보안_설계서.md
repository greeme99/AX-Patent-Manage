...
## 2. 목적
특허/설계 비밀정보와 승인증적을 보호하는 인증·인가·데이터·AI·운영 통제를 정의한다.
...
## 5. 보안 통제
### 인증/인가
- Enterprise SSO, MFA는 IdP 정책
- RBAC + Project Membership + Need-to-Know
- ROLE-LEGAL/EXEC의 승인 권한 별도
- 서비스 계정 최소권한

### 데이터
- TLS 1.2+ / at-rest encryption
- Object storage signed URL short TTL
- Evidence hash 및 무결성 확인
- secret/classification label
- export/download audit

### 애플리케이션
- OWASP ASVS 기반 검증
- CSRF/XSS/SQLi/SSRF 방어
- rate limit
- secure headers
- dependency/SAST/DAST

### AI
- 승인되지 않은 외부 모델로 Confidential 데이터 전송 금지
- Prompt injection 방어: source/tool allowlist
- RAG ACL pre-filter
- AI run audit: model, prompt, inputs ref, outputs ref
- Legal conclusion guardrail

### 운영
- 관리자 행위 감사
- 로그에 claim/evidence 원문 최소화
- 정기 권한 재검토
- Incident response 및 credential rotation
...
## 9. 완료 기준
인증·인가·데이터·앱·AI·운영 보안 통제가 아키텍처와 일치.
